<script>
    ...
    const migrate_menus = async function(){
        ...
        for (let i = 0; i < menus.length; i++){
            ...
            const new_menu_data = {
                ...
                "servings": build_servings(menus[i].servings),
                "description": menus[i].description,
                "notes": validate_notes(menus[i].notes),
                "grocery_list": "RELATION_RECORD_ID",
                "made": "JSON",
                "sub_recipes": [
                    "RELATION_RECORD_ID"
                ],
            ...
            console.log({new_menu_data})
            // const new_menus = await pb.collection('menus_strict').create(new_menu_data);
        }
    }

    const build_grocery_list = function(in_list){

    }

    const build_servings = function(in_serv){

    }

    const validate_notes = function(in_notes){
    ...
    const get_new_recipe = async function(in_id){
        ...
        if (new_recipe.length == 1){
            return new_recipe[0];
        }else if (!new_recipe.length){
    ...
    const delete_table = async function (table) {
    ...
    {#if is_auth_user}
        <div class="flex flex-auto flex-wrap justify-around items-center space-y-96 md:space-y-12 pt-36 pb-16 md:pt-0 md:pb-0 h-full">
                ...
                {#each tables as table}
                    <button id="compare_parsers_btn" class="btn btn-primary w-56" on:click={() => {delete_table(table)}}>
                        clear {table}
                    </button>
                {/each}
                <div>
