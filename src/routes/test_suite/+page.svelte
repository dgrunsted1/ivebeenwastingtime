...
async function get_recipes(num){
    const recipe_links = await pb.collection('recipes').getFullList({expand:`ingr_list`});
    console.log({recipe_links});
    return shuffle(recipe_links).slice(0, num);
}
...
async function process_recipe_test(e){
    ...
    const recipe_links = await get_recipes(num_tests);
    e.srcElement.firstChild.value = `10`;
    process_recipe_results = [];
    console.log({recipe_links});
    for (let i = 0; i < recipe_links.length; i++){
        console.log(i, recipe_links[i]);
        const data = new FormData(this);
        data.append("recipe", JSON.stringify(recipe_links[i]));
        console.log("recipe data", data);
        let scraped_ingr = [];
        const response = await fetch(this.action, {
            method: 'POST',
            body: data,
            recipe: recipe_links[i]
        });
        ...
        if (result.data.err) {
            ...
            process_recipe_results  = [test_recipe(scraped_recipe, recipe_links[i])].concat(process_recipe_results);    
                    ...
                    <!-- <h3 class="text-center">NPM : MINE</h3> -->
                        {#each process_recipe_results as recipe}
                            {#if recipe.passed == true}
                                <div class="bg-error-content text-success rounded text-center col-span-3"><div>{recipe.title}</div><a href={recipe.url}>{recipe.url}</a><div>{recipe.scrape_time}</div><div>{recipe.id}</div></div>
                            {:else}
                                <div class="bg-error-content text-error rounded text-center col-span-3"><div>{recipe.title}</div><a href={recipe.url}>{recipe.url}</a><div>{recipe.scrape_time}</div><div>{recipe.id}</div></div>
                            {/if}
